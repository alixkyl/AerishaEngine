...
module.exports.generate = function(options){
	...
	function patchKnot(x,y){
		return simplex.noise2D(x, y)+landSea;
	}
	/**
	*nurbsGenerator
	*/
	function nurbsGenerator(x,y,size){
		var nsControlPoints = [
			[
				new THREE.Vector4 ( 0, patchKnot(x, y), 0, 1 ),
				new THREE.Vector4 ( 0, patchKnot(x, y+size/2), 5, 1 ),
				new THREE.Vector4 ( 0, patchKnot(x, y+size), 10, 1 )
			],
			[
				new THREE.Vector4 ( 5, patchKnot(x+size/2, y), 0, 1 ),
				new THREE.Vector4 ( 5, patchKnot(x+size/2, y+size/2), 5, 1 ),
				new THREE.Vector4 ( 5, patchKnot(x+size/2, y+size), 10, 1 )
			],
			[
				new THREE.Vector4 ( 10, patchKnot(x+size, y), 0, 1 ),
				new THREE.Vector4 ( 10, patchKnot(x+size, y+size/2), 5, 1 ),
				new THREE.Vector4 ( 10, patchKnot(x+size, y+size), 10, 1 )
			]
		];
		var degree1 = 2;
		var degree2 = 2;
